...
namespace agencyData.DataSource.Savings
{
    [DataObject]
    public class AgencySavingsStatementDataSource
    {
        ...
        public AgencySavingsStatementDataSource()
        {
        ...
        public SavingsStatementViewModel GetClientStatement(int savingId, DateTime startDate, DateTime endDate)
        {
            ...
            if (statementDetail.transactions.Count > 0)
            {
                ...
                statementDetail.balanceAtEnd = statementDetail.transactions.OrderByDescending(p =>p.date)
            ...
            foreach (var transac in statementDetail.transactions)
            {
                ...
                else { transac.strDepositAmount = transac.depositAmount.ToString("#,##0.00"); }
                if (transac.withdrawalAmount < 1) { transac.strWithdrawalAmount = ""; }
                else { transac.strWithdrawalAmount = transac.withdrawalAmount.ToString("#,##0.00"); }
            }
