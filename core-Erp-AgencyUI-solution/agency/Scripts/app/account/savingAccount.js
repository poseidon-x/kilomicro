...
var savingApiUrl = agencyAPI_URL_Root + "/Saving";
var savingTypeApiUrl = agencyAPI_URL_Root + "/SavingType";
var clientApiUrl = agencyAPI_URL_Root + "/AllClients";
var savingPlanIntervalApiUrl = agencyAPI_URL_Root + "/SavingPlanInterval";
var depositRepaymentModeApiUrl = agencyAPI_URL_Root + "/DepositRepaymentMode";
var fieldAgentsApiUrl = agencyAPI_URL_Root + "/Agent";
...
var onSavingAccount = function(){
	var savTypId = $("#savingType").data("kendoComboBox").value();
	var monthlyRate = $("#monthlyInterestRate").data("kendoNumericTextBox");
	
	var exist = false;
	
	//Retrieve value enter validate
    for (var i = 0; i < savingTypes.length; i++) {
        if (savingTypes[i].savingTypeID == savTypId) {
			monthlyRate.value(savingTypes[i].interestRate);
            exist = true;
			break;
        }
    }
...
function saveToServer() {	
    ...
    $.ajax({
        ...
        dismissLoadingDialog();
        successDialog('Savings Account Successfully Saved', 'SUCCESS', function () { window.location = '/Agent/Dashboard'; });        
