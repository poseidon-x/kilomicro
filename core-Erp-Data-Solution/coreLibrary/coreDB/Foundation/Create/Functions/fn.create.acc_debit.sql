...
AS
BEGIN
	declare @debit float, @earning float
	...
	select @debit = sum(
			case when @cat_code in (1,5,6,8) then crdt_amt   else dbt_amt end
		)
	...
	select @debit = ISNULL(@debit, 0) 
	if @earning<0
	begin
		select @debit= @debit - @earning
	end
	return @debit
END
GO
