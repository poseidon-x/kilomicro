...
SELECT        pm.payMasterID, pc.payCalendarID, pm.basicSalary, pm.netSalary, pc.year, pc.month, pc.isProcessed, pc.isPosted, s.staffNo, s.staffID, 
                         ...
                         ISNULL(s.DOB, GETDATE()) AS DOB, sc.staffCategoryID, sc.staffCategoryName, pma.description, pma.employeeAmount, pma.employerAmount, 
						 pma.pensionTypeID AS typeID, 
...
where ssn is not null and len(ltrim(rtrim(ssn))) > 0
