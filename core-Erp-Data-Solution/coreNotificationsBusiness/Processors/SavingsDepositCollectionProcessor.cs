...
{
    public class SavingsDepositCollectionProcessor : IEventLoanRepaymentsCollectionProcessor
    {
        private const int SAVINGS_APPROVAL_EVENT_CATEGORY_ID = 3;
        ...
        public int messageEventCategoryID
        {
            get
            {
                return SAVINGS_APPROVAL_EVENT_CATEGORY_ID;
            } 
        }
        ...
        public void process()
        {
                    ...
                    using (var ne = new coreNotificationsDAL.notificationsModel())
                    {
                        if (ne.messageEventCategories.FirstOrDefault(p => p.messageEventCategoryID == messageEventCategoryID).isEnabled == true)
                        {
                            ...
                            foreach (var r in deposits)
                            {
                                ...
                                if (mobileNo != "")
                                {
                                    var bal = GetBalanceAsAt(r.savingDate, r.savingID, conf);
                                    if (!r.posted)
                                    {
                                        bal = bal + r.savingAmount;
                                    }
                                    var body = template.messageBodyTemplate.Replace("$$ACCOUNT_NUMBER$$", r.saving.savingNo.AccountNumber())
                                        .Replace("$$AMOUNT$$", r.savingAmount.ToString("#,##0.#0"))
                                        ...
                                        .Replace("$$DATE$$",r.savingDate.ToString("yyyy-MM-dd"))
                                        .Replace("$$NARATION$$", r.naration.Truncate(config.maxNarationLength));
                                    ne.messageEvents.Add(new coreNotificationsDAL.messageEvent
                                    {
                                        accountID = r.savingID,
                                        clientID = r.saving.clientID,
                                        eventDate = DateTime.Now,
                                        eventID = r.savingAdditionalID,
            ...
            {
                ExceptionManager.LogException(x, "SavingsDepositCollectionProcessor.process");
            }
        }

        private List<savingAdditional> getNewDeposits(coreLoansEntities le, coreNotificationsDAL.notificationsModel ne)
        {
            List<savingAdditional> deposits = new List<savingAdditional>();
            var startDate = DateTime.Now.Date;
            var endDate = DateTime.Now;

            var apps = le.savingAdditionals.Where(p => p.savingDate >= startDate && p.savingDate <= endDate
                && p.posted == true).ToList();
            foreach (var r in apps)
            {
                var ev = ne.messageEvents.FirstOrDefault(p => p.eventID == r.savingAdditionalID
