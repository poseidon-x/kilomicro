...
namespace coreErpApi.Controllers.Controllers.Deposits
{
    ...
    public class DepositTypeController : ApiController
    {
        ...
        // GET: api/depositType
        [HttpGet]
        public IEnumerable<depositType> GetDepositUpgradeableDepositTypes(int id)
        {
            var deposit = le.deposits.Include(p => p.depositType).FirstOrDefault(p => p.depositID == id);
            if (deposit == null) return null;
            double dpBal = deposit.principalBalance + deposit.interestBalance;
            var data = le.depositTypes
                .Where(p => deposit.depositType.ordinal < p.ordinal
                && dpBal  >=  p.upgradeMinimumAmount)
                .OrderBy(p => p.depositTypeName)
                .ToList();
