...
namespace coreERP.Controllers
{
    ...
    [AuthorizationFilter()]
    public class ControllerFileController : ApiController
    {
        ...
        public ControllerFileController()
        {
        ...
        [HttpGet]
        public List<ControllerFileVM> GetAllControllerFile()
        {
            //get an entry with its id
            List<ControllerFileVM> value = null;
            value = le.controllerFiles
        ...
        [HttpGet]
        public List<controllerFileDetail> GetControllerFileDetails(int id)
        {
            //get an entry with its id
            List<controllerFileDetail> value = null;
            value = le.controllerFileDetails
                .Where(p => p.fileID == id)
                .ToList();
            return value;
        }
        ...
        [HttpPost]
        public bool  ControllerFileUpdate(ControllerFileVM ln)

        {
            ...
            foreach (var controllerFileDetail in ln.controllerFileDetails)
            {
                ...
                controllerFileDetail toBeSaved = null;
                if (controllerFileDetail.fileDetailID> 0)
                {
                    toBeSaved = le.controllerFileDetails
        ...
        private void populateFields(controllerFileDetail target, controllerFileDetail source)
        {
            if(source.loanNo != null)
            {
                var loanDetails = le.loanNo_by_staffID.FirstOrDefault(p => p.loanNo == source.loanNo);
                var loanId = loanDetails.loanID;
                var RepSchd = le.repaymentSchedules.FirstOrDefault(p => p.loanID == loanId && p.principalBalance>0);
                var RepSchdId = RepSchd.repaymentScheduleID;

                target.overage = source.overage;
                target.loanNo = source.loanNo;
                target.remarks = source.remarks;
                target.controllerRepaymentTypeId = source.controllerRepaymentTypeId;
                target.repaymentScheduleID = RepSchdId;
            }
            else
            {
                target.overage = source.overage;
        ...
        [HttpGet]
        public List<ControllerFileMonDedVM>GetControllerFileOutstanding(int id)
        {
            var query = le.vw_controllerFile_outstandingLoan
                    .Where(p => p.fileID == id)
                    .Select(p => new ControllerFileMonDedVM
                    {
                        amountDisbursed = p.amountDisbursed,
                        monthlyDeduction = p.monthlyDeduction,
                        overage = p.overage,
                        loanNo = p.loanNo,
                        remarks = p.remarks,
                        disbursementDate = p.disbursementDate,
                        fileDetailID = p.fileDetailID,
                        fileID = p.fileID,
                        loanID = p.loanID,
        ...
        {
            target.amountDisbursed = source.amountDisbursed;
