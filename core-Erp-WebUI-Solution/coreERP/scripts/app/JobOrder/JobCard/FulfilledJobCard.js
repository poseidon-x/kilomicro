...
function prepareUi() {
    ...
    $('#fulfill').click(function (event) {
        ...
        if (jobCard[0].approved && !jobCard[0].fulfilled) {
            if (confirm('Are you sure you want to Mark Job card as fulfilled?')) {
                displayLoadingDialog();
                saveFulfillment();
            } else {
                smallerWarningDialog('Job card approved or fulfilled already', 'NOTE');
            }
        }
    });
}
...
function populateUi() {
    $('#workOrderNumber').data('kendoComboBox').value(jobCard[0].workOrderNumber);
    $('#jobDate').data('kendoDatePicker').value(jobCard[0].jobDate);
    $('#orderStartingDate').data('kendoDatePicker').value(jobCard[0].orderStartingDate);
    $('#customer').data('kendoComboBox').value(jobCard[0].customerId);
    $('#standardMarkUpRate').data('kendoNumericTextBox').value(jobCard[0].standardMarkUpRate);
    $('#standardHourlyBillingRate').data('kendoNumericTextBox').value(jobCard[0].standardHourlyBillingRate);
    $('#totalLabour').data('kendoNumericTextBox').value(jobCard[0].totalLabour);
    $('#totalMaterial').data('kendoNumericTextBox').value(jobCard[0].totalMaterial);
    var vatNNhil = jobCard[0].vat + jobCard[0].nhil;
        ...
        dataSource: {
            transport: {
                read: function (entries) {
                    entries.success(jobCard[0].jobCardLabourDetails);
                },
...
function renderMaterialGrid() {
    $('#materialDetailGrid').kendoGrid({
        dataSource: {
            transport: {
                read: function (entries) {
                    entries.success(jobCard[0].jobCardMaterialDetails);
                },
                create: function (entries) {
                    entries.success(entries.data);
                },
...
function saveToServer() {
    ...
    $.ajax({
        url: jobCardApiUrl + '/FulfillJobCard',
        type: 'Post',
        contentType: 'application/json',
        data: JSON.stringify(jobCard[0]),
        beforeSend: function (req) {
            req.setRequestHeader('Authorization', "coreBearer " + authToken);
        }
