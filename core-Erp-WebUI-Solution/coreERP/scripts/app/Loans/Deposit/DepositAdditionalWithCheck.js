...
var depositAdditionalApiUrl = coreERPAPI_URL_Root + "/crud/depositAdditionalWithCheck";
...
var depositAdditionalAjax = $.ajax({
    url: depositAdditionalApiUrl + '/Get/' + id,
    type: 'Get',
...
var depositAjax = $.ajax({
    url: depositApiUrl + '/GetDeposit/' + depId,
    type: 'Get',
    beforeSend: function (req) {
        req.setRequestHeader('Authorization', "coreBearer " + authToken);
    }
...
//Load page data
function loadData() {	
    $.when(depositAdditionalAjax,depositAjax,depositTypeAjax,clientAjax,currencyAjax,
	bankAjax,paymentModeAjax,currencyNoteAjax,cashierFundAjax,cashierFundTodayAjax)
        .done(function (dataDepositAdditional,dataDeposit,dataDepositType,dataClient,
		dataCurrency,dataBank,dataPaymentMode,dataCurrencyNote,dataCashierFund,dataCashierFundToday) {
            depositAdditional = dataDepositAdditional[2].responseJSON;
            currentDeposit = dataDeposit[2].responseJSON;
			...
			if(cashierFundToday!=null){
				...
				if(currentDeposit.depositID > 0){
					depositAdditional.deposit = currentDeposit;
					setUpAccountDeatils();
				}else{
					allowClientSearch()
				}
...
function prepareUi(){
    ...
    $('#save').click(function (event) {
		if (confirm("Are you sure you want deposit into Investment account")) {
            displayLoadingDialog();
			saveWithdrawal();				
		} else
		{
...
function renderControls() {	
	...
	.kendoComboBox({
		...
		dataTextField: "depositAccountNo",
		filter: "contains",
		highlightFirst: true,
		suggest: true,
		ignoreCase: true,
	...
	$("#avaialablePrincipalBalance").width("90%")
	.kendoNumericTextBox();
	$("#avaialableInterestBalance").width("90%")
	.kendoNumericTextBox();
...
function setUpAccountDeatils(){
	...
	getClientPicture(currentDeposit.clientID);
	populateUi();
	var depos = {
		depositId:currentDeposit.depositID,
		depositAccountNo:currentDeposit.depositNo
	};
	var client = currentDeposit.client;
	var cln = {
	...
	depo.value(currentDeposit.depositID);
...
function populateUi(){
	var dep = depositAdditional.deposit;
...
function resetControls(){
	$("#depositAmount").data("kendoNumericTextBox").value("");	
	$("#tabs").hide();
	$("#save").hide();
	...
	$("#depositDate").data("kendoDatePicker").value("");		
	$("#chequeDetails").hide();
}
...
function renderGrid(){
	$('#notes').kendoGrid({
        dataSource: {
                transport: {
					read: function (entries) {
							entries.success(depositAdditional.depositAdditionalNotes);                    
					},
					create: function (entries) {
						var data = entries.data;						
						data.depositAdditionalNoteId = noteAdditionalId;
						noteAdditionalId += -1;
                ...
                schema: {
                    model: {
                    id: 'depositAdditionalNoteId',
                    fields: {
						depositAdditionalNoteId: { type: 'number' },
						depositAdditionalID: { type: 'number' },
        ...
        columns: [
            ...
            { field: 'quantityDeposited', title: 'Pieces Deposited', editor: quantityDepositedEditor, format: "{0: #,###}" },  
            ...
            { field: 'totalDeposited', title: 'Amount Deposited (GHS)',editor: totalDepositedEditor, format: "{0: #,###.#0}" },            
	...
	$('#coins').kendoGrid({
        dataSource: {
                transport: {
					read: function (entries) {
							entries.success(depositAdditional.depositAdditionalCoins);                    
					},
					create: function (entries) {
						var data = entries.data;
						data.depositAdditionalCoinId = coinAdditionalId;
						coinAdditionalId += -1;
						data.quantityDeposited = $("#quantityDep").data("kendoNumericTextBox").value();
						data.quantityCD = $("#quantityCD").data("kendoNumericTextBox").value();
						data.quantityBD = $("#quantityDB").data("kendoNumericTextBox").value();
                ...
                schema: {
                    model: {
                    id: 'depositAdditionalCoinId',
                    fields: {
						depositAdditionalCoinId: { type: 'number' },
						depositAdditionalID: { type: 'number' },
						currencyNoteId: { editable: true, validation: { required: true } },
						quantityDeposited: { type: 'number', validation: { required: true, min: 1 } },
						quantityBD: { type: 'number', editable:true,validation: { required: true } },
...
function getClientDepositAccount(id){
	$.ajax({
        ...
        depositAdditional.deposit = data;
		populateUi();
    }).error(function (xhr, data, error) {
		dismissLoadingDialog();
		//warningDialog(xhr.responseJSON.ExceptionMessage,'ERROR'); 
...
function retrieveValues(){
	...
	depositAdditional.depositAdditional.modeOfPaymentID = paymentType;
	depositAdditional.depositAdditional.naration = $("#naration").data("kendoMaskedTextBox").value();
	depositAdditional.depositAdditional.depositDate = $("#depositDate").data("kendoDatePicker").value();	
	depositAdditional.depositAdditional.depositAmount = $("#depositAmount").data("kendoNumericTextBox").value();
	
	if(paymentType==2){
			depositAdditional.depositAdditional.checkNo = $("#checkNo").data("kendoMaskedTextBox").value();
			depositAdditional.depositAdditional.bankID = $("#bank").data("kendoComboBox").value();
	}else{
		depositAdditional.depositAdditionalNotes = $('#notes').data().kendoGrid.dataSource.view();
		depositAdditional.depositAdditionalCoins = $('#coins').data().kendoGrid.dataSource.view();
	}
}
...
function saveToServer() {	
    ...
    $.ajax({
        url: depositAdditionalApiUrl + '/PostDepositAdditional',
        type: 'Post',
        contentType: 'application/json',
        data: JSON.stringify(depositAdditional),
        beforeSend: function(req) {
            req.setRequestHeader('Authorization', "coreBearer " + authToken);
        }
    }).success(function (data) {
        ...
        dismissLoadingDialog();
        successDialog('Investment Deposit Successful', 'SUCCESS',
		function () { window.location = '/ln/depositReports/addReceipt.aspx?id='+data.depositAdditionalID; });        
    }).error(function (xhr, data, error) {
		dismissLoadingDialog();
		warningDialog(xhr.responseJSON.ExceptionMessage,'ERROR'); 
	});	
...
function getTotalDeposit(){
	var dpAmt = $("#depositAmount").data("kendoNumericTextBox");
	var tot = 0;
