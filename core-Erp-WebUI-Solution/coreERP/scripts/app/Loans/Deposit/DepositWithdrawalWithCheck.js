...
var depositWithdrawalApiUrl = coreERPAPI_URL_Root + "/crud/DepositWithdrawalWithCheck";
...
var depositWithdrawalType = 'N';

var noteWithdrawalId = -1;
var coinWithdrawalId = -1;

var depositWithdrawalAjax = $.ajax({
    url: depositWithdrawalApiUrl + '/Get/' + id,
    type: 'Get',
...
//Load page data
function loadData() {	
    $.when(depositWithdrawalAjax,depositAjax,depositTypeAjax,clientAjax,currencyAjax,
	bankAjax,paymentModeAjax,currencyNoteAjax,cashierFundAjax,cashierFundTodayAjax)
        .done(function (dataDepositWithdrawal,dataDeposit,dataDepositType,dataClient,
		dataCurrency,dataBank,dataPaymentMode,dataCurrencyNote,dataCashierFund,dataCashierFundToday) {
            depositWithdrawal = dataDepositWithdrawal[2].responseJSON;
			...
			if(cashierFundToday!=null){
				...
				if(deposit.depositID > 0){
					depositWithdrawal.deposit = deposit;
					setUpAccountDeatils();
				}else{
					allowClientSearch()
				}
...
function prepareUi(){
    ...
    $('#save').click(function (event) {
		...
		}else {warningDialog("Please make sure all required fields are provided",'WARNING');}
	});
	
	$('input[type=radio][name=withdrawalType]').change(function() {
		var amt = $("#withdrawalAmount").data("kendoNumericTextBox");
		var dp = depositWithdrawal.deposit;
        if (this.value == 'interest') {
			amt.max(dp.interestBalance);
			depositWithdrawalType = 'I';
        }
        else if (this.value == 'principal') {
			amt.max(dp.principalBalance);
			depositWithdrawalType = 'P';
        }
		else if (this.value == 'both') {
			amt.max(dp.principalBalance+dp.interestBalance);
			depositWithdrawalType = 'B';
        }
	});
}
