...
function getDueApprovals() {
	...
	displayLoadingDialog();	
	$.ajax({
	    url: LoanGroupBatchApprovalApiUrl + "/GetGroupsLoanPendingCheckList",
		type: 'POST',
...
function renderGrid() {
    $('#approvalGrid').kendoGrid({
		...
		columns: [
            ...
            {
                field: 'approved', title: 'Checked ?',
                template: '<input type="checkbox" #= approved ? \'checked="approved"\' : "" # disabled="disabled" />'
            },
			{ command: [{ name: "edit", text: "Mark Checklist Done" }], width: '20%' },
...
function saveToServer(approvalData) {
    ...
    $.ajax({
        url: LoanGroupBatchApprovalApiUrl + "/PostBatchChecklist",
		type: 'POST',
        contentType: "application/json",
		...
		dismissLoadingDialog();
		successDialog('Loan Checklist Successfully Recieved',
