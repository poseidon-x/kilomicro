...
var LoanGroupPostApiUrl = coreERPAPI_URL_Root + "/crud/LoanGroupPost";
...
function prepareUi() 
    ...
    if (validator.validate()) {
		...
		if(gridData.length > 1 && gridData.length < 31){
		    ...
		    smallerWarningDialog('Group clients cannot be less than two (2) or more than 30 ', 'Warning');
		}
...
function renderGrid() {
    $('#groupGrid').kendoGrid({
			...
			pageable: {
				pageSize: 50,
				pageSizes: [50, 100, 150, 500, 1000,1500],
				previousNext: true,
				buttonCount: 5
			},
			sortable: {
