...
function prepareUi() {
    ...
    renderControls();
        ...
        var btn = '<input type="button" class="btn btn-primary" id="post" value="Post Balance" />';
        var msg = '<font color="red" face="verdana">Balance Posted!</font>';

        if (openningBalanceBatch.posted == false) {
            $('#postButton').html(btn);
        } else {
            $('#postButton').html(msg);
        }
        ...
        $('#post').click(function (event) {
            if (openningBalanceBatch.approved) {
                if ($("#postingComments").val() == "") {
                    ...
                    if (confirm('Are you sure you want Post this balance?')) {
                        displayLoadingDialog();
                        saveApproval();
                    } else {
...
function populateUi() {
    ...
    if (openningBalanceBatch.posted) {
        $('#postingComments').data('kendoEditor').value(openningBalanceBatch.postingComments);
...
function retrieveValues() {
    openningBalanceBatch.postingComments = $('#postingComments').data('kendoEditor').value();
}

//func saveToServer
function saveToServer() {
    $.ajax({
        url: openningBalanceBatchApiUrl + '/BalancePost',
        type: 'Post',
        contentType: 'application/json',
        ...
        dismissLoadingDialog();
        successDialog('Balance Posted Successfuly', 'SUCCESS', function () { window.location = '/OpeningBalances/OpenningBalances/'; });
