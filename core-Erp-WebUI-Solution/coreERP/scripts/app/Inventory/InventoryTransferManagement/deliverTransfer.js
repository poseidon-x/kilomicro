...
function prepareUi() {
    ...
    populateUi();
    renderGrid();
    var bt = '<input type="button" class="btn btn-primary" id="deliver" value="Mark As Delivered" />';
    var msg = '<font color="red" face="verdana">Transfer Delivered!</font>';
    if (inventoryTransfer.delivered == false) {
        $('#deliveredButton').html(bt);
    } else {
        $('#deliveredButton').html(msg);
    }
    ...
    $('#deliver').click(function (event) {
        ...
        if (!validator.validate()) {
            $("#deliveredBy").css("border", "solid 1px #ff0000");
            $("#receivedBy").css("border", "solid 1px #ff0000");
        } else {
            if (confirm('Are you sure you want mark this Transfer as delivered?')) {
                displayLoadingDialog();
                saveTransfer();
            } else {
                smallerWarningDialog('Please review and mark as delivered later', 'NOTE');
            }
        }
...
function renderControls() {
    $('#deliveredBy').kendoMaskedTextBox({
    });

    $('#receivedBy').kendoMaskedTextBox({
    });

    $("#deliverersComments").kendoEditor();

    $("#recieversComments").kendoEditor();
...
function retrieveValues() {
    inventoryTransfer.deliveredBy = $('#deliveredBy').data('kendoMaskedTextBox').value();
    inventoryTransfer.receivedBy = $('#receivedBy').data('kendoMaskedTextBox').value();
    if (!$('#deliverersComments').val == "") {
        inventoryTransfer.deliverersComments = $('#deliverersComments').data('kendoEditor').value();
    }
    if (!$('#recieversComments').val == "") {
        inventoryTransfer.recieversComments = $('#recieversComments').data('kendoEditor').value();
    }
}
...
function saveToServer() {
    ...
    $.ajax({
        url: inventoryTransferApiUrl + '/PostDelivery',
        type: 'Post',
        contentType: 'application/json',
